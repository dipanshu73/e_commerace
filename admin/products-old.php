...
<div class="container mt-5">
    ...
    <?php
        ...
        if (isset($_POST['save_product'])) {
            ...
            if (!empty($_POST['edit_id'])) {
                ...
                $update_sql = "UPDATE products 
                               SET product_name='$product_name', category_id='$category_id', price='$price', display_location='$display_location' 
                               WHERE id='$id'";
                ...
                $insert_sql = "INSERT INTO products (product_name, category_id, price, display_location, created_at) 
                               VALUES ('$product_name', '$category_id', '$price', '$display_location', NOW())";
        ...
        $sql = "SELECT p.id, p.product_name, c.category_name, p.price, p.created_at, p.category_id, p.display_location 
                FROM products p 
        ...
        <div class="row g-3">
                ...
                <select name="category_id" class="form-select">
                    ...
                    <?php
                        ...
                        while($cat = mysqli_fetch_assoc($cat_result)) {
                            $selected = ($edit_mode && $cat['id'] == $edit_category) ? "selected" : "";
            ...
            <div class="col-md-2">
                ...
                <input type="text" name="price" class="form-control" 
                       value="<?php echo $edit_mode ? $edit_price : ''; ?>" required>
            </div>
            <div class="col-md-2">
                <label class="form-label fw-bold">Display Location</label>
                <select name="display_location" class="form-select">
                    <option value="category_list" <?php if($edit_display=='category_list') echo 'selected'; ?>>Show in Category List</option>
                    <option value="separate_list" <?php if($edit_display=='separate_list') echo 'selected'; ?>>Show in Separate List</option>
                    <option value="hidden" <?php if($edit_display=='hidden') echo 'selected'; ?>>Hide for now</option>
                </select>
            </div>
            <div class="col-md-2">
                <button type="submit" name="save_product" class="btn btn-<?php echo $edit_mode ? 'warning' : 'primary'; ?> w-100 mt-4">
            ...
            <?php 
                if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    <tr>
                        ...
                        <td>
                            <?php 
                                if($row['display_location']=='category_list') echo "Category List";
                                elseif($row['display_location']=='separate_list') echo "Separate List";
                                else echo "Hidden";
                            ?>
                        </td>
                        <td><?php if(isset($row['created_at'])){ echo  date("d-m-Y H:i A", strtotime($row['created_at'])); } ?></td>
                        <td>
